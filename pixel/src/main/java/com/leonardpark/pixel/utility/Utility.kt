  ...
  companion object {
    ...
    fun getImageVideoCursor(context: Context, excludeVideo: Boolean): Cursor? {
      return context.contentResolver
        .query(
          Constants.IMAGE_VIDEO_URI,
          Constants.IMAGE_VIDEO_PROJECTION,
          if (excludeVideo) Constants.IMAGE_SELECTION else Constants.IMAGE_VIDEO_SELECTION,
          null,
          Constants.IMAGE_VIDEO_ORDERBY
        )
    }
    ...
    fun manipulateVisibility(
      ...
      if (slideOffset > 0 && recyclerView.visibility == View.INVISIBLE) {
        ...
        status_bar_bg.animate().translationY(-status_bar_bg.height.toFloat()).setDuration(550)
          .start()
      }
    }
